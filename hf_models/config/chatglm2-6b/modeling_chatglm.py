...
class RotaryEmbedding(nn.Module):
    ...
    def forward_impl(
        ...
        n_elem_and_base_str = "-".join([str(n_elem), str(base)])
        if n_elem_and_base_str not in self.theta_map:
            theta = 1.0 / (base ** (torch.arange(0, n_elem, 2, dtype=dtype, device='cpu') / n_elem)).to(device)
            self.theta_map[n_elem_and_base_str] = theta
        else:
            theta = self.theta_map[n_elem_and_base_str]
