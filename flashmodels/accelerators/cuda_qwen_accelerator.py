...
class CUDAQwenAccelerator(Accelerator):
    ...
    def get_qwen_cls(self, class_name="QWenBlock"):
        module_path = "transformers_modules/modeling_qwen"
    ...
    def apply_checkpointing(self, model):
        qwen_block = self.get_qwen_cls()

        non_reentrant_wrapper = functools.partial(
            checkpoint_wrapper,
            checkpoint_impl=CheckpointImpl.NO_REENTRANT,
        )
        check_fn = lambda submodule: isinstance(submodule, qwen_block)
...
AcceleratorFactory.regist("cuda-qwen", CUDAQwenAccelerator)
