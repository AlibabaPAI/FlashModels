...
class CUDAOlmoAccelerator(Accelerator):
    ...
    def get_olmo_cls(self):
        import olmo
        return olmo.OlmoSequentialBlock

    def apply_checkpointing(self, model):
        olmo_block = self.get_olmo_cls()

        non_reentrant_wrapper = functools.partial(
            checkpoint_wrapper,
            checkpoint_impl=CheckpointImpl.NO_REENTRANT,
        )
        check_fn = lambda submodule: isinstance(submodule, olmo_block)
...
AcceleratorFactory.regist("cuda-olmo", CUDAOlmoAccelerator)
