...
class ACCGLMAccelerator(Accelerator):
    ...
    def get_config(self, model):
        ...
        if self.args.gc:
            config.memory.gc_cls = {"GLMBlock"}

        config.dist.fsdp.size = self.args.fsdp_num
        config.dist.fsdp.wrap_layer_cls = {"GLMBlock"}
...
AcceleratorFactory.regist("acc-glm", ACCGLMAccelerator)
