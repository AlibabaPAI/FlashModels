...
class CUDAGemmaAccelerator(Accelerator):
    """ This is the Distributed Accelerator specifically for Gemma,
    which is implmented with Pytorch and transformer APIs.
    """
    def accelerate(self, model, loader):
        ...
        if self.args.fsdp_num == 1 and self.args.tp_num == 1 and self.args.gc:
    ...
    def fsdp(self, model):
        ...
        model = FSDP(
            ...
            use_orig_params=self.args.lora,
        )
...
AcceleratorFactory.regist("cuda-gemma", CUDAGemmaAccelerator)
