...
class CUDAGLMAccelerator(Accelerator):
    ...
    def get_glm_cls(self, class_name="GLMBlock"):
        module_path = "transformers_modules/modeling_chatglm"
    ...
    def apply_checkpointing(self, model):
        glmblock = self.get_glm_cls("GLMBlock")

        non_reentrant_wrapper = functools.partial(
            checkpoint_wrapper,
            checkpoint_impl=CheckpointImpl.NO_REENTRANT,
        )
        check_fn = lambda submodule: isinstance(submodule, glmblock)
...
AcceleratorFactory.regist("cuda-glm", CUDAGLMAccelerator)
